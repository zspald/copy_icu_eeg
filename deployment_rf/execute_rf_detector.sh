...
declare -a PATIENTS=(
				...
				### ICUDataRedux_0091)
# declare -a PATIENTS=(ICUDataRedux_0061 ICUDataRedux_0063)
num_pts=${#PATIENTS[@]}
num_par=4
# echo $length
...
for (( i=0; i<${num_pts}; i+=4));
	do
		...
		for pt in ${temp[@]};
			do
				# echo $pos
				python rf_detector.py -u $username -p $password -id $pt -b $bipolar -po $pool -rb $ref_and_bip -t $threshold -l $length -pos $pos &
				((pos+=1))
			done
			wait
			pos=0
...
# 	echo $patient
	# python rf_detector.py -u $username -p $password -id $patient -b $bipolar -po $pool -rb $ref_and_bip -t $threshold -l $length
# done
wait
